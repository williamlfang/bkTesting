...
#
## -----------------------------------------------------------------------------
png(paste0("../fig/",file_name,"_基金产品净值曲线_",
           data_name,".png"), res = 72*3, height=1500*0.9, width=1500*1.6)
...
plot(draw.down.infor$Date, -draw.down.infor$Per_Draw_Down, type = "s",
     ...
     ylim = c(min(-draw.down.infor$Per_Draw_Down)*3,0),
     col = "cyan2", lwd = 1)
axis(4)
...
legend(x = (dataInfor[1,Date]-86400*150), y = dataInfor[,max(NAV,NAV_SH_index,NAV_HS300_index)*0.75],legend = c('回撤(%)','上证综指','沪深300','基金净值'),
